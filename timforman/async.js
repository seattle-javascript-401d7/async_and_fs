...
fs.readFile('one.txt', function (err, one) {
  if (err) return err;
  fs.writeFile('results.txt', one.toString('hex', 0, 8), (err) => {
...
function second() {
  fs.readFile('two.txt', function (err, two) {
    if (err) return err;
    fs.appendFile('results.txt', two.toString('hex', 0, 8), (err) => {
...
function third() {
  fs.readFile('three.txt', function (err, three) {
    if (err) return err;
    fs.appendFile('results.txt', three.toString('hex', 0, 8), (err) => {
